...
The Windows tray form writes `<stem>_talk_sheet.json` (open title, overview, Point 1–3 platform/still/optional "Starts when I say" cue/image title/image text/card titles/card headlines, source flags). Close and identity stay locked. Point picture windows lock to the trimmed transcript: a filled start cue, or the first solid hit of that point's card title / body / platform. A missing cue skips that point's picture (full-frame host) instead of reusing Point 1 cards. Empty slots may auto-fill a short image title from the platform or still name, and image text from the point. Auto copy never invents a directive or says DoD. Auto-fill never splits a user-typed image text string. User slots do not get rewritten.
